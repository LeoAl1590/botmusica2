        ...
        if (client.settings.get(message.guild.id, `botchannel`).length > 0) {
            if (!client.settings.get(message.guild.id, `botchannel`).includes(message.channel.id) && !message.member.permissions.has("ADMINISTRATOR")) {
               return message.reply({
                 ...
                 .setTitle(`${client.allEmojis.x} **No podes usar el comando acá**`)
                  .setDescription(`Usalo en estos canales:\n> ${client.settings.get(message.guild.id, `botchannel`).map(c=>`<#${c}>`).join(", ")}`)
                ]
               })
            }
        }
        ...
        try {
          ...
          if(command.minargs && command.minargs > 0 && args.length < command.minargs) {
            return message.reply({embeds: [new Discord.MessageEmbed()
              .setColor(ee.wrongcolor)
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(`${client.allEmojis.x} Comando erróneo`)
              .setDescription(command.argsmissing_message && command.argsmissing_message.trim().length > 0 ? command.argsmissing_message : command.usage ? `Uso: ` + command.usage : `Comando erróneo`)]
          ...
          if(command.maxargs && command.maxargs > 0 && args.length > command.maxargs) {
            return message.reply({embeds: [new Discord.MessageEmbed()
              ...
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(`${client.allEmojis.x} Comando erróneo`)
              .setDescription(command.argstoomany_message && command.argstoomany_message.trim().length > 0 ? command.argstoomany_message : command.usage ? `Uso: ` + command.usage : `Comando erróneo`)]
          ...
          if(command.maxplusargs && command.maxplusargs > 0 && args.join(` `).split(`++`).filter(Boolean).length > command.maxplusargs) {
            return message.reply({embeds: [new Discord.MessageEmbed()
              ...
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(`${client.allEmojis.x} Comando erróneo`)
              .setDescription(command.argstoomany_message && command.argstoomany_message.trim().length > 0 ? command.argsmissing_message : command.usage ? `Usage: ` + command.usage : `Comando erróneo`)]
