...
function customplaylistembed(lyrics, song) {
    if (!lyrics) lyrics = "No hay canciones en la playlist";
    try {
        let embeds = [];
        let k = 1000;
    ...
    try {
        ...
        for (let i = 0; i < lyrics.length; i += 1000) {
            ...
            const embed = new Discord.MessageEmbed()
                ...
                .setFooter(`Canción pedida por: ${song.user.tag}`, song.user.displayAvatarURL({dynamic: true}))
                .setColor(ee.color)
                .setDescription(current)
            embeds.push(embed);
        }
...
function onCoolDown(message, command) {
   ...
   */
   ...
   async function swap_pages(client, message, description, TITLE) {
     ...
     let button_back = new MessageButton().setStyle('SUCCESS').setCustomId('1').setEmoji("833802907509719130").setLabel("Atras")
     let button_home = new MessageButton().setStyle('DANGER').setCustomId('2').setEmoji("🏠").setLabel("Inicio")
     let button_forward = new MessageButton().setStyle('SUCCESS').setCustomId('3').setEmoji('832598861813776394').setLabel("Adelante")
     ...
     let swapmsg = await message.channel.send({   
         content: `***Toca los botones para cambiar de página***`,
         embeds: [embeds[0]], 
     ...
     collector.on('collect', async b => {
         if(b.user.id !== message.author.id)
           return b.reply(`<:declined:780403017160982538> **Solo el que escribió ${prefix}help puede reaccionar**`, true)
           //page forward
           if(b.customId == "1") {
